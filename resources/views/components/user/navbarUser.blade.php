<nav class="bg-gradient-to-r to-emerald-600 from-sky-400 border-gray-200 dark:bg-gray-900">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="{{ route('dashboardUser') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img src="{{ url('/image/Group 10.png') }}" class="h-8" alt="Flowbite Logo" />
            <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">FasilReads</span>
        </a>
        <div class="flex items-center  md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            <button type="button"
                ...
                @if (!auth()->user()->profile)
                    <img class="w-8 h-8 rounded-full" src="{{ asset('image/profile.png') }}" alt="user photo" />
                @else
                    <img class="w-8 h-8 rounded-full" src="{{ asset('storage/' . auth()->user()->profile) }}"
                        alt="user photo" />
                @endif
            </button>
            ...
            <!-- Dropdown menu -->
            <div class=" mx-4 z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600"
                id="user-dropdown">
                <div class="px-4 py-3">
                    ...
                    @auth
                        ...
                        <span
                            class="block text-sm text-gray-500 truncate dark:text-gray-400">{{ auth()->user()->nimNip }}</span>
                    @endauth
                </div>
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-user">
            <ul
                class="flex flex-col font-medium p-4 md:p-0 sm:mt-4 md:mt-0 border border-gray-100 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:border-0 d
                ...
                <li>
                    <a href="{{ route('pengingatKembaliPeminjaman') }}"
                ...
                <div class="flex md:order-2">
                    ...
                    <button data-collapse-toggle="navbar-search" type="button"
                        class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
                        aria-controls="navbar-search" aria-expanded="false"></button>
                </div>
            </ul>
        </div>
    </div>
